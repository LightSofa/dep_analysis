...
class PluginSettings:
    ...
    def __init__(self, organizer: mobase.IOrganizer, plugin_name: str):
        ...
        # 从 MO2 的设置系统中读取值
        self.CACHE_EXPIRATION_DAYS: int = self._organizer.pluginSetting(self._plugin_name, "cache_expiration_days")
        self.REQUEST_TIMEOUT: int = self._organizer.pluginSetting(self._plugin_name, "request_timeout")
        self.AUTO_OPEN_REPORT: bool = self._organizer.pluginSetting(self._plugin_name, "auto_open_report")
...
class ModAnalyzer:
    ...
    def __init__(self, organizer: mobase.IOrganizer, settings: PluginSettings, log_emitter: pyqtSignal):
    ...
    def _initialize_browser(self) -> bool:
        ...
        if self.settings.COOKIES_PATH.exists():
            try:
                ...
                cookies = None
                self.log("缓存的cookies.json文件无效，将尝试其他方法。", "warning")

        # 备用：如果JSON加载失败，则尝试转换MO2的.dat文件
        if not cookies:
            print("Chrome浏览器安装")
            ...
            # 如果转换成功，将其保存为JSON格式以备将来使用
            if cookies:
                self._save_playwright_cookies_to_json(cookies)

        # 如果所有方法都失败了
        if not cookies:
            self.log("未找到任何有效的Cookies源。", "error")
            return False
        ...
        try:
            ...
            self.log("浏览器启动并登录成功。")
            return True
        except PLAYWRIGHT_MODULE.sync_api.Error as e:
            self.log(f"初始化浏览器时发生Playwright错误: {e}", "critical")
            if self.playwright: self.playwright.stop()
            return False
        except Exception as e:
            self.log(f"初始化浏览器或登录时发生未知错误: {e}", "critical")
            if self.playwright: self.playwright.stop()
            return False

    def _load_cookies_from_file(self) -> Optional[List[Dict]]:
        if not self.settings.COOKIES_PATH.exists() or self.settings.COOKIES_PATH.stat().st_size == 0:
            return None
        try:
            with open(self.settings.COOKIES_PATH, 'r', encoding='utf-8') as f:
                cookies = json.load(f)
            self.log(f"成功从 '{self.settings.COOKIES_PATH.name}' 加载Cookies。")
            return cookies
        except (json.JSONDecodeError, IOError) as e:
            self.log(f"加载或解析Cookies文件 '{self.settings.COOKIES_PATH.name}' 时出错: {e}。", "warning")
            return None
    ...
    def get_mod_data(self, mod_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            def scrape_dep_section(header_text: str) -> List[Dict[str, str]]:
                dependencies = []
                header = soup.find('h3', string=lambda t: t and header_text.lower() in t.lower().strip())
